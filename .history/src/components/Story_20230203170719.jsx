...
const Story = ({ username, img }) => {
  ...
  return (
    <div className="box-border flex flex-col truncate flex-shrink-0 items-center justify-center h-30 w-24">
      <div
        className={`relative ${
          username === session?.user.username ? "opacity-60" : "opacity-100"
        }`}>
        <LazyLoadImage
          alt={username}
          effect="blur"
        ...
        {username === session?.user.username && (
          <PlusIcon className="h-14 w-14 font-bold absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] text-white" />
        )}
      </div>

...
const Story = ({ username, img }) => {
  ...
  return (
    <div className="box-border flex flex-col truncate flex-shrink-0 items-center justify-center h-30 w-24">
      <div
        className={`relative ${
          username === session?.user.username ? "opacity-100" : "opacity-100"
        }`}>
        <LazyLoadImage
          alt={username}
          effect="blur"
          src={img}
          className={`opacity-40 h-[70px] border-2 cursor-pointer border-orange-600 p-[2px] w-[70px] self-center rounded-full`}
        />

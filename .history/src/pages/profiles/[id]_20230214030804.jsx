...
import { Cog6ToothIcon, HeartIconFilled } from "@heroicons/react/24/solid";
...
const Page = ({ id }) => {
  ...
  useEffect(() => {
    if (activePhoto) {
      const unsubscribe = onSnapshot(
        collection(db, "posts", activePhoto.id, "likes"),
        (snapshot) => {
  ...
  /* const likePost = async () => {
    if (hasLiked) {
      await deleteDoc(
        doc(db, "posts", activePhoto.id, "likes", currentUser?.uid)
      );
    } else {
      await setDoc(
        doc(db, "posts", activePhoto.id, "likes", currentUser?.uid),

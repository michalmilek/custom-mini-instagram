...
const Page = ({ id }) => {
  ...
  return (
    <div className="flex flex-col items-center min-w-[100vw]">
      ...
      <section className="grid grid-cols-4 gap-3 w-screen items-center justify-center px-12">
        {posts.map((post) => (
          <LazyLoadImage
            src={post.data().image}
            alt={post.data().caption}
            className="h-[400px] w-[400px] box-border border-2 border-black rounded-md"
          />
        ))}
      </section>
    </div>
